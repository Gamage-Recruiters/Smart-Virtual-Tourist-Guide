...
export const createBooking = async (req, res) => {
    try {
        const HotelBookingModel = await getModel();
        const booking = await HotelBookingModel.create(req.body);
...
export const updateBooking = async (req, res) => {
    try {
        ...
        const existingBooking = await HotelBookingModel.findById(id).select('status');
        ...
        const booking = await HotelBookingModel.findByIdAndUpdate(id, req.body, { new: true, runValidators: true });
...
export const getBookingsByHotel = async (req, res) => {
    try {
        ...
        const bookings = await HotelBookingModel.find({ hotelId }).sort({ bookedDate: -1 });
