...
const cleanList = (value) => Array.isArray(value) ? value.map(String).map((item) => item.trim()).filter(Boolean) : [];
...
const totalsFor = (request, bid) => {
  ...
  const dailyRate = Number(bid.amount);
  const subtotal = Number((dailyRate * days).toFixed(2));
...
export const listMyRequests = async (req, res, next) => {
  try {
    ...
    const bidCounts = await GuideBid.aggregate([{ $match: { request: { $in: ids } } }, { $group: { _id: '$request', count: { $sum: 1 } } }]);
...
export const cancelMyRequest = async (req, res, next) => {
  try {
    ...
    if (!guideRequest) return fail(res, 404, 'Guide request not found');
    if (!sameId(guideRequest.tourist, req.user._id)) return fail(res, 403, 'You do not own this guide request');
    if (['booked', 'completed'].includes(guideRequest.status)) return fail(res, 409, 'A booked or completed request cannot be cancelled here');
...
export const listRequestBids = async (req, res, next) => {
  try {
    ...
    if (!guideRequest) return fail(res, 404, 'Guide request not found');
    if (!sameId(guideRequest.tourist, req.user._id)) return fail(res, 403, 'You do not own this guide request');
    const bids = await GuideBid.find({ request: guideRequest._id }).sort({ amount: 1 }).populate('guide', 'fullName username guideId').lean();
...
export const listOpportunities = async (req, res, next) => {
  try {
    ...
    const requests = await GuideRequest.find({ status: { $in: ['open', 'receiving_bids'] }, _id: { $nin: myBidRequestIds }, endDate: { $gte: new Date() } }).sort({ createdAt: -1 }).populate('tourist', 'fullName').lean();
  ...
  try {
    ...
    try {
      const bid = await GuideBid.create({
        ...
        currency: String(req.body.currency || guideRequest.currency || 'LKR').toUpperCase(), proposal: req.body.proposal.trim(),
...
export const updateProviderBookingStatus = async (req, res, next) => {
  try {
    ...
    if (!['confirmed', 'rejected', 'cancelled', 'completed'].includes(status)) return fail(res, 400, 'Invalid booking status');
    if (['cancelled', 'rejected', 'completed'].includes(booking.status)) return fail(res, 409, 'This booking status is final');
...
export const dashboard = async (req, res, next) => {
  try {
    const [profile, packages, bookings, opportunities, earningStats] = await Promise.all([
      GuideProfile.findOne({ user: req.user._id }).lean(),
      GuidePackage.find({ guide: req.user._id }).lean(),
      GuideBooking.find({ guide: req.user._id }).sort({ createdAt: -1 }).limit(8).populate('tourist', 'fullName').lean(),
      GuideRequest.find({ status: { $in: ['open', 'receiving_bids'] }, endDate: { $gte: new Date() } }).sort({ createdAt: -1 }).limit(5).lean(),
    ...
    res.json({ success: true, summary: { totalEarnings, availablePayout: totalEarnings, activeTours: packages.filter((item) => item.status === 'published').length, averageRating: profile?.ratingAverage || 0 }, latestRequests: opportunities, bookings });
  } catch (error) { next(error); }
};
...
const packagePayload = (body) => ({
  ...
  stops: cleanList(body.stops), images: cleanList(body.images), pricePerPerson: Number(body.pricePerPerson),
...
export const earnings = async (req, res, next) => {
  try {
    ...
    const totalEarnings = transactions.reduce((sum, item) => sum + item.total, 0);
    const pending = await GuideBooking.find({ guide: req.user._id, paymentStatus: 'pending', status: { $ne: 'cancelled' } }).populate('tourist', 'fullName').lean();
    res.json({ success: true, summary: { totalEarnings, availablePayout: totalEarnings, pendingPayout: pending.reduce((sum, item) => sum + item.total, 0), completedBookings: transactions.length }, transactions });
  } catch (error) { next(error); }
};

export const getMyProfile = async (req, res, next) => {
  try { const profile = await GuideProfile.findOne({ user: req.user._id }).lean(); res.json({ success: true, user: { _id: req.user._id, fullName: req.user.fullName, email: req.user.email, contactNumber: req.user.contactNumber, guideId: req.user.guideId, dob: req.user.dob, gender: req.user.gender }, profile: profile || null }); } catch (error) { next(error); }
};
...
export const listPublicGuides = async (_req, res, next) => {
  try {
    ...
    res.json({ success: true, guides: users.map((user) => ({ user: safeUser(user), guideId: user.guideId || '', ...(safeProfile(byUser[String(user._id)]) || {}) })) });
  } catch (error) { next(error); }
};

export const getPublicGuide = async (req, res, next) => {
  try { if (!isId(req.params.guideId)) return fail(res, 400, 'Invalid guide ID'); const guide = await profileFor(req.params.guideId); if (!guide) return fail(res, 404, 'Guide not found'); res.json({ success: true, guide }); } catch (error) { next(error); }
};
