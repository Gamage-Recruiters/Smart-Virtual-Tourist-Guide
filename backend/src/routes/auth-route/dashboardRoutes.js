...
import { protect, authorizeRoles } from '../../middleware/auth-middleware/authMiddleware.js';
...
// Activity Provider dashboard route (protected & restricted to 'activityprovider_user')
router.get(
  '/activity-provider',
  protect,
  authorizeRoles('activityprovider_user'),
  (req, res) => {
    res.json({
      success: true,
      message: 'Welcome to the Activity Provider Dashboard',
      user: req.user,
    });
  }
);
