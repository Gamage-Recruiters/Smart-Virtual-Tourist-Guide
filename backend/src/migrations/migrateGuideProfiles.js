...
import normalizeGuideFields from './normalizeGuideFields.js';

const collectionIndexes = async (collection) => {
...
const dropIndexes = async (collection, names) => {
  const existing = new Set((await collectionIndexes(collection)).map((index) => index.name));
