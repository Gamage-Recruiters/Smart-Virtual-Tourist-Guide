...
import { DESCRIPTION_MAX_LENGTH } from '../../utils/guideValidation'
    ...
    <form onSubmit={onSubmit} noValidate className="space-y-6">
      <fieldset className="guide-card"><legend className="guide-legend">Trip details</legend>
        <div className="grid gap-5 md:grid-cols-2">
          <Field label="Starting location" name="startLocation" error={errors.startLocation} required>{({ className, errorId }) => <input className={className} value={values.startLocation} onChange={setValue('startLocation')} aria-invalid={Boolean(errors.startLocation)} aria-describedby={errors.startLocation ? errorId : undefined} />}</Field>
          <Field label="Main destination" name="destination" error={errors.destination} required>{({ className, errorId }) => <input className={className} value={values.destination} onChange={setValue('destination')} aria-invalid={Boolean(errors.destination)} aria-describedby={errors.destination ? errorId : undefined} />}</Field>
          <Field label="Additional destinations or planned stops" name="stops">{({ className }) => <input className={className} value={values.stops} onChange={setValue('stops')} placeholder="Dambulla, Kandy" />}</Field>
          ...
          <Field label="Pickup location" name="pickupLocation">{({ className }) => <input className={className} value={values.pickupLocation} onChange={setValue('pickupLocation')} />}</Field>
          <Field label="Drop-off location" name="dropoffLocation">{({ className }) => <input className={className} value={values.dropoffLocation} onChange={setValue('dropoffLocation')} />}</Field>
        </div>
      </fieldset>
      ...
      <fieldset className="guide-card"><legend className="guide-legend">Guide preferences</legend>
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <Field label="Additional languages" name="additionalLanguages">{({ className }) => <input className={className} value={values.additionalLanguages} onChange={setValue('additionalLanguages')} placeholder="French, German" />}</Field>
          <Field label="Minimum experience (years, optional)" name="minExperience">{({ className }) => <input className={className} type="number" min="0" value={values.minExperience} onChange={setValue('minExperience')} />}</Field>
          <Field label="Minimum rating (optional)" name="minRating">{({ className }) => <select className={className} value={values.minRating} onChange={setValue('minRating')}><option value="">Any rating</option><option value="4">4.0+</option><option value="4.5">4.5+</option><option value="4.8">4.8+</option></select>}</Field>
        </div>
      ...
      <fieldset className="guide-card"><legend className="guide-legend">Additional information</legend><div className="grid gap-5 md:grid-cols-2">
        ...
        <Field label="Special requirements" name="specialRequirements">{({ className }) => <textarea className={`${className} min-h-32 py-3`} value={values.specialRequirements} onChange={setValue('specialRequirements')} />}</Field>
        <Field label="Mobility or accessibility needs" name="accessibilityNeeds">{({ className }) => <textarea className={`${className} min-h-24 py-3`} value={values.accessibilityNeeds} onChange={setValue('accessibilityNeeds')} placeholder="Only share details needed to plan the tour" />}</Field>
