...
import { fetchPlacesVisitedCount } from '../services/itineraryService';

const TripHighlights = ({ touristId, tripId }) => { 
    const [placesVisited, setPlacesVisited] = useState(0);

    useEffect(() => {
        const getVisitedCount = async () => {
            if (!touristId || !tripId) return;
            try {
         
                const result = await fetchPlacesVisitedCount(touristId, tripId);
                
                if (result.success) {
                    setPlacesVisited(result.count); 
                }
            } catch (error) {
