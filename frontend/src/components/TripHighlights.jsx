...
import { fetchTripStats } from '../services/itineraryService';

const TripHighlights = ({ touristId, tripId }) => {

    const [totalPlaces, setPlacesVisited] = useState(0);
    const [totalPhotos, setPhotos] = useState(0);
    ...
    useEffect(() => {
        const getTripStats = async () => {
            ...
            try {
                ...
                const result = await fetchTripStats(touristId, tripId);

                if (result.success) {
                    setPlacesVisited(result.data.totalPlaces);
                    setPhotos(result.data.totalPhotos);
                    setFood(result.data.totalFood);
                    setDistance(result.data.totalDistance);
                }
            } catch (error) {
                console.error("Error fetching places visited count:", error);
            }
        };
