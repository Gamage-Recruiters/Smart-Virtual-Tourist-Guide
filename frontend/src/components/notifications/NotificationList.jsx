...
// 🚀 IMPORT THE NEW CUSTOM HOOK HERE
...
const NotificationList = () => {
  ...
  // 🚀 GET THE HOOK FUNCTION (Renamed to 'handleViewDetails' to avoid confusion)
  const { handleNotificationClick: handleViewDetails } = useNotificationNavigation();
  ...
  // Flatten the pages array to get a single list of notifications
  const list = useMemo(() => {
    return data?.pages.flatMap((page) => page.data) || [];
  }, [data]);

  // Mutation to mark a notification as read in the database
  const markAsReadMutation = useMutation({
    ...
    onMutate: async (notificationId) => {
      ...
      // Update the local cache immediately for a faster UI feel
    ...
    onError: (_err, _variables, context) => {
      // Revert back to old data if API call fails
  ...
  // Expand the card to show full message and mark it as read
  ...
  return (
    ...
    >
      {/* Show skeletons while loading first time */}
      {isLoading && (
        ...
        {list.map((notification) => {
          ...
          return (
            <div
              ...
              onClick={() => handleCardClick(notification)} // Expand card on click
              // Glassmorphism and soft shadows for the card
              className={`group relative flex flex-col p-4 cursor-pointer rounded-2xl transition-all duration-300 border backdrop-blur-sm ${
                notification.isRead
                  ...
                  : `bg-blue-50/60 border-blue-100 shadow-sm hover:shadow-md hover:bg-blue-50/90 ${getLeftBorderColor(
                      notification.priority,
                    )}`
              } ${
                isExpanded
                  ? "ring-2 ring-[#00aaff]/30 bg-white scale-[1.01] shadow-lg"
                  : ""
              }`}
            >
              <div className="flex items-start">
                {/* Icon Container */}
                <div
                  className={`shrink-0 p-2.5 rounded-full mr-4 ring-4 ring-white shadow-sm ${getIconColor(
                    notification.priority,
                  )}`}
                >
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <div className="flex items-start justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2 shrink-0 pt-0.5">
                      ...
                      {/* Pinging blue dot for unread messages */}
                      {!notification.isRead && (
                  ...
                  <p
                    className={`text-xs leading-relaxed transition-all duration-300 ${
                      isExpanded
                        ? "text-slate-700"
                        : "text-slate-500 line-clamp-2 font-medium"
                    }`}
                  >
                    {notification.message}
                  </p>

                  {/* Expanded Actions (View Details Button) */}
                  {isExpanded && (
                      ...
                      {notification.actionUrl && (
                        <button
                          onClick={(e) => {
                            ...
                            // 🚀 USE THE HOOK TO NAVIGATE (Closes modal & goes to URL)
      ...
      {/* Show skeleton while loading more pages */}
      {isFetchingNextPage && (
        <div className="mt-3">
