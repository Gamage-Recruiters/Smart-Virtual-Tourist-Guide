  ...
  return <div className="min-h-screen bg-[#EBF1FF] p-4 font-sans text-gray-800 sm:p-6">
    <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-6 lg:grid-cols-4">
      ...
      <section className="space-y-6 lg:col-span-3" aria-labelledby="public-guides-title">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3"><div className="flex space-x-2 text-xs font-bold"><button type="button" className="rounded-xl bg-blue-600 px-5 py-2.5 text-white shadow-sm">All Guides</button><button type="button" onClick={() => navigate('/guide-bids')} className="rounded-xl border border-gray-100 bg-white px-5 py-2.5 text-gray-500 hover:bg-gray-50">Custom Offers</button></div><div className="flex items-center gap-3"><span className="text-xs font-bold text-gray-500">{guides.length} packages</span><select aria-label="Sort guides" value={filters.sort} onChange={(event) => setFilters((current) => ({ ...current, sort: event.target.value }))} className="min-h-10 rounded-xl border border-gray-200 bg-white px-3 text-xs font-bold text-gray-600"><option value="recommended">Recommended</option><option value="rating">Highest rating</option><option value="price-low">Price: low to high</option><option value="price-high">Price: high to low</option></select></div></div>
