...
import addpackages from '../../assets/HotelOwner/add-special-packages-image.png';
import toproom from '../../assets/HotelOwner/toproom.png';
import specialPak from '../../assets/HotelOwner/specialPak.png';
...
export default function AddSpecialPackages() {
  ...
  useEffect(() => {
    if (!editId) return;
    fetch(`${BASE_URL}/api/special-packages/${editId}`)
      .then((r) => r.json())
      .then(({ package: pkg }) => {
        if (!pkg) return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isEditMode ? `${BASE_URL}/api/special-packages/${editId}` : `${BASE_URL}/api/special-packages`;
                  ...
                  <div className="space-y-3">
                    ...
                    <div className={`flex overflow-hidden border bg-white ${fieldErrors.basePrice ? 'border-red-500' : 'border-slate-300'}`}>
                      <div className="flex-1 px-4 py-3">
                        <input type="number" name="basePrice" value={formData.basePrice} onChange={handleChange} onWheel={(e) => e.target.blur()} placeholder="$150" className="w-full bg-transparent text-sm text-slate-600 placeholder-slate-400 focus:outline-none" />
                      </div>
