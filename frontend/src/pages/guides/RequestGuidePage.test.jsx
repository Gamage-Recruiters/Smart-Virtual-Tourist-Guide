...
describe('RequestGuidePage', () => {
  ...
  it('shows a sign-in action when the API returns 401', async () => {
    const user = userEvent.setup()
    vi.spyOn(guideService, 'createRequest').mockRejectedValue(Object.assign(new Error('Authentication is required.'), { status: 401 }))
    renderPage()
    await user.type(screen.getByLabelText(/Starting location/), 'Colombo')
    await user.type(screen.getByLabelText(/Main destination/), 'Sigiriya')
    await user.type(screen.getByLabelText(/Start date/), '2026-11-12')
    ...
    await user.type(screen.getByLabelText(/Maximum budget/), '30000')
    await user.click(screen.getByRole('button', { name: 'Submit request' }))
    expect(await screen.findByText(/Your session is missing or expired/)).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'Sign in and try again.' })).toHaveAttribute('href', '/login')
  })
})
