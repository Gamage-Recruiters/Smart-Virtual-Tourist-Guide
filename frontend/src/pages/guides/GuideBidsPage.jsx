...
export default function GuideBidsPage() {
  ...
  if (flow.error) return <GuideLayout><h1 className="mb-6 text-3xl font-extrabold">Available Guides</h1><GuideEmptyState title={flow.errorStatus === 401 ? 'Please sign in to view guide bids' : flow.errorStatus === 403 ? 'You do not have access to this request' : 'We could not load guide bids'} description={flow.error} actionLabel={flow.errorStatus === 401 ? 'Go to login' : 'Retry'} onAction={flow.errorStatus === 401 ? () => navigate('/login', { state: { returnTo: `/guides/requests/${requestId}/bids` } }) : flow.retry} /></GuideLayout>
  ...
  const disabledRequest = flow.request.status !== 'Open'
