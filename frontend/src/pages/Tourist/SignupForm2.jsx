...
const TravelSafetyInfo = () => {
  ...
  useEffect(() => {
    // Check if user has Form 1 data or existing profile
    const signupData = localStorage.getItem('signupData')
    const existingProfile = localStorage.getItem('touristProfile')
  ...
  // Update handleSubmit to properly structure and store the data
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const fullTouristProfile = {
        fullName: signupData.fullName || '',
        email: signupData.email || '',
        gender: signupData.gender || 'Male',
        country: signupData.country || 'Sri Lanka',
        travelType: signupData.travelType || 'Solo',
      ...
      try {
        const response = await userAPI.register({
          ...
          travelPreferences: {
            travelStart: formData.travelStart,
            travelEnd: formData.travelEnd,
            budgetRange: { min: budgetMin, max: budgetMax, currency: 'LKR' },
            travelStyle: formData.travelStyle,
            accommodationType: formData.accommodationType
          },
          healthInfo: {
        ...
        console.warn('API registration notice:', apiError);
      }

      // Save complete tourist profile locally so Profile page can display and edit it
      localStorage.setItem('touristProfile', JSON.stringify(fullTouristProfile));
      localStorage.setItem('profileCompleted', 'true');
      localStorage.setItem('userData', JSON.stringify({
        fullName: fullTouristProfile.fullName,
        email: fullTouristProfile.email,
        country: fullTouristProfile.country
      }));
      ...
      localStorage.setItem('tripInfo', JSON.stringify({
        startDate: fullTouristProfile.travelStart,
        endDate: fullTouristProfile.travelEnd,
        budgetLKR: budgetMax,
        preferences: fullTouristProfile.travelStyle
      }));

      // Seed MongoDB with the tourist's budget immediately after registration
      if (fullTouristProfile.travelStart && fullTouristProfile.travelEnd && budgetMax > 0) {
        try {
          await fetch('http://localhost:5000/api/budget/optimize', {
            method: 'POST',
          ...
          console.log(`[SignupForm2] Budget plan generated: LKR ${budgetMax.toLocaleString()} for userId ${userId}`);
        } catch (budgetErr) {
          console.warn('[SignupForm2] Could not pre-generate budget plan:', budgetErr.message);
        }
      }
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <div className="flex gap-4 items-center">
                          <span className="text-sm text-gray-600">Min: Rs. {budgetMin.toLocaleString()}</span>
                          <input
                            type="range"
                            min="0"
                            max="500000"
                        ...
                        <div className="flex gap-4 items-center">
                          <span className="text-sm text-gray-600">Max: Rs. {budgetMax.toLocaleString()}</span>
                          <input
                            type="range"
                            min="0"
                            max="1000000"
                        ...
                        <div className="text-center p-3 bg-blue-50/90 rounded-lg" style={{ borderRadius: '10px' }}>
                          <span className="font-semibold text-[#3CB4FF]">Selected Budget: Rs. {budgetMin.toLocaleString()} - Rs. {budgetMax.toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
